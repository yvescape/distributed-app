...
/opt/venv/bin/python payement/manage.py makemigrations --noinput

echo "[entrypoint] DB prête. Migration..."
/opt/venv/bin/python payement/manage.py migrate --noinput

echo "[entrypoint] Création du superuser si inexistant..."
/opt/venv/bin/python payement/manage.py shell -c "
...
/opt/venv/bin/python payement/manage.py collectstatic --noinput

echo "[entrypoint] Démarrage Gunicorn..."
exec /opt/venv/bin/gunicorn --config payement/gunicorn.conf.py